...
export default {
  ...
  methods: {
    getBatchAll() {
      let url=`v1/batch/query_all?type=${this.type}`;
    ...
    uploadFile() {
      ...
      let that = this
      this.file = null
      let requestUrl='loan_applications/upload_validate/';
      if(this.requestUrl&&this.requestUrl.import)
      {
        if(this.requestUrl.import.url){
          requestUrl=this.requestUrl.import.url;
          Object.entries(this.requestUrl.import.params).map(([index,value])=>{
            formData.append(index,value);
          })
        }else{
          requestUrl=this.requestUrl.import;
        } 
        
      }
      if(this.parenttype){
        requestUrl=`v1/${requestUrl}${this.parenttype}/${this.type}`
      }else{
    ...
    downloadTemplate() {
      ...
      if(this.requestUrl&&this.requestUrl.exportUrl){
        if(this.requestUrl.exportUrl.params){
          exportUrl=this.requestUrl.exportUrl.url;
          params={...params,...this.requestUrl.exportUrl.params};
        }else{
          exportUrl=this.requestUrl.exportUrl;
        }
      }else{
    ...
    downloadResult(batchId) {
      let result='loan_applications/exportBatchResult';
      if(this.requestUrl&&this.requestUrl.result)
      {
        result=this.requestUrl.result;
      }
      exportData({ batchId, type: this.type }, `v1/${result}`)
    },
    remove(batchId) {
      let that = this
      ...
      that.$Modal.confirm({
        ...
        content: '确定删除?',
        onOk: () => {
          axios.get('v1/batch/delete_one_record',{
            params: {
              batchId
            }
          }).then(function() {
