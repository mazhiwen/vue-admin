...
export default {
          ...
          render: (h, params) => {
            if(params.row.state === '已完成') {
              return h('a', {
                  ...
                  on: {
                    click: () => {
                      this.downloadResult(params.row)
                    }
                  }
                }, '下载')
            }
  ...
  methods: {
    getBatchAll() {
      axios.get(this.requestData.getBatchAll.url).then((res) => {
    ...
    uploadFile() {
      ...
      let that = this
      this.file = null
      let url=this.requestData.import.url;
    ...
    downloadTemplate() {
      exportData(this.requestData.exportUrl.params, this.requestData.exportUrl.url)
    },
    downloadResult(row) {
      let params={};
      Object.entries(this.requestData.result.params).map(([index,value])=>{
        params[index]=row[value];
      })
      exportData(params, this.requestData.result.url)
    },
    remove(row) {
      let params={};
      Object.entries(this.requestData.delete.params).map(([index,value])=>{
        params[index]=row[value];
      })
      this.$Modal.confirm({
        title: '提示',
        content: '确定删除?',
        onOk: () => {
          axios.get(this.requestData.delete.url,{
            params
          }).then(()=> {
  ...
  watch:{
    requestData:function(newValue,oldValule){
